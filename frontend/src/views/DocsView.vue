<template>
  <div class="docs-layout">
    <!-- 上传区域 (仅 admin) -->
    <div v-if="auth.isAdmin" class="upload-zone" @click="triggerUpload">
      ...
      <div v-else class="tree-body">
        <div v-for="group in docGroups" :key="group.name" class="category-group">
          ...
          <div v-for="doc in group.files" :key="doc.fullpath" class="doc-row">
            ...
            <el-button v-if="auth.isAdmin" type="danger" link size="small" :icon="Delete" @click.stop="delDoc(doc.fullpath)" />
          </div>
        </div>
      </div>
    </div>
